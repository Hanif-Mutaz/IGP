...
function removeBundle(i) {
    if (DB.bundleRows.length <= 1) { toast('Minimal 1 bundle', 'error'); return; }
...
function submitBuatPO() {
    ...
    if (!total || !konsumen) { toast('Konsumen dan produk wajib dipilih', 'error'); return; }
    if (!salesNama) { toast('Sales wajib dipilih sebelum membuat PO', 'error'); return; }
    if (totalBundle <= 0) { toast('Jumlah bundle tidak valid', 'error'); return; }

    // Konfirmasi dulu sebelum PO dibuat
    const ringkasan = DB.bundleRows.map(r => `${r.produk} x${r.qty}`).join(', ');
    if (!confirm(`Konfirmasi buat PO?\n\nKonsumen: ${konsumen.nama}\nProduk: ${ringkasan}\nTotal: ${fmtRpFull(total)}\n\nData tidak bisa diubah setelah PO dibuat.`)) return;
...
function hapusPO(id) {
    const p = DB.poList.find(x => x.id === id);
    if (!p) return;
    if (p.status !== 'lunas' && p.status !== 'retur') {
        toast('Hanya PO yang sudah lunas atau retur yang bisa dihapus', 'error');
        return;
    }
    konfirmasiHapus(
        'Hapus PO',
        `Hapus "${p.id}" (${p.konsumen}) secara permanen?`,
        () => {
            ...
            // Recalculate konsumen.tagihan dari PO yang tersisa (bukan dari p.sisa yang stale)
            if (k) {
                k.tagihan = DB.poList
            ...
            saveDB();
            toast(`PO ${id} dihapus`);
            addAudit(`Hapus PO: ${id}`);
            renderPOList();
            if (DB.selectedPO) renderPODetail(DB.selectedPO);
            else { document.getElementById('po-detail-header').innerHTML = ''; document.getElementById('po-detail-body').innerHTML = '<div class="empty">Pilih PO dari daftar</div>'; }
        }
    );
}
...
function cairkanKomisi(id) {
    const e = DB.entitas.find(x => x.id === id);
    if (!e) return;
    const outstanding = Math.max(0, (e.komisiKotor || 0) - (e.komisiDibayar || 0));
    if (outstanding <= 0) { toast('Tidak ada komisi yang perlu dicairkan', 'warn'); return; }

    // Validasi: Sales/Nego hanya boleh cairkan jika ada PO yang sudah mencapai split milestone
    ...
    if (!confirm(`Cairkan komisi ${e.nama}?\n\nJumlah: ${fmtRpFull(outstanding)}\n\nIni menandai komisi sudah dibayarkan ke ${e.nama}. Aksi tidak dapat dibatalkan.`)) return;
    e.komisiDibayar = (e.komisiDibayar || 0) + outstanding;
    if (!e.riwayatCair) e.riwayatCair = [];
    e.riwayatCair.push({ tanggal: formatDateShort(new Date().toISOString().split('T')[0]), jumlah: outstanding });
    saveDB();
...
function cetakKwitansiKomisi(id) {
    const e = DB.entitas.find(x => x.id === id);
    if (!e) return;
    const prs = DB.settings.perusahaan || {};
    const nPrs = prs.nama || 'INTERGAS PERDANA';
    ...
    const today = new Date();
    const tglCetak = `${today.getDate()} ${months[today.getMonth()]} ${today.getFullYear()}`;
    const noKwt = `KMK/${today.getFullYear()}${String(today.getMonth() + 1).padStart(2, '0')}/${String(e.id).padStart(4, '0')}`;

    // Hitung komisi berdasarkan peran
    ...
    function poInBulanLocal(po) { return poMatchFilter(po, bulanSel, tahunSel); }
    ...
    if (e.peran === 'Sales') {
        const poS = DB.poList.filter(p => p.sales === e.nama && poInBulanLocal(p));
        const rate = DB.settings.komisi_sales || 0;
        const bundleTotal = poS.reduce((s, p) => s + p.bundle, 0);
        totalKomisi = bundleTotal * rate;
        detailRows = `<tr><td>${e.peran}</td><td>${bundleTotal} bundle terjual</td><td style="text-align:right">${fmtRpFull(rate)}/bundle</td><td style="text-align:right;font-weight:600;color:#1B3A5C">${fmtRpFull(totalKomisi)}</td></tr>`;
    } else if (e.peran === 'Nego') {
        const poN = DB.poList.filter(p => p.nego === e.nama && poInBulanLocal(p));
        const rate = DB.settings.komisi_nego || 15000;
        totalKomisi = poN.length * rate;
        detailRows = `<tr><td>${e.peran}</td><td>${poN.length} PO dinego</td><td style="text-align:right">${fmtRpFull(rate)}/PO</td><td style="text-align:right;font-weight:600;color:#1B3A5C">${fmtRpFull(totalKomisi)}</td></tr>`;
    } else if (e.peran === 'Collector') {
        const poColl = DB.poList.filter(p => p.coll === e.nama && poInBulanLocal(p));
        const rate = DB.settings.komisi_coll || 1500;
        const totalTx = poColl.reduce((s, p) =>
            s + p.cicilan.filter(c => c.status === 'lunas' || c.status === 'kurang' || (c.terbayar || 0) > 0).length, 0);
        totalKomisi = totalTx * rate;
        detailRows = `<tr><td>${e.peran}</td><td>${totalTx} kwitansi tertagih</td><td style="text-align:right">${fmtRpFull(rate)}/kwitansi</td><td style="text-align:right;font-weight:600;color:#1B3A5C">${fmtRpFull(totalKomisi)}</td></tr>`;
    } else if (e.peran === 'Kepala Cabang') {
        const poLunas = DB.poList.filter(p => p.status === 'lunas' && poInBulanLocal(p));
        const rate = DB.settings.komisi_kc || 5000;
        const bundleTotal = poLunas.reduce((s, p) => s + p.bundle, 0);
        totalKomisi = bundleTotal * rate;
        detailRows = `<tr><td>${e.peran}</td><td>${bundleTotal} bundle lunas</td><td style="text-align:right">${fmtRpFull(rate)}/bundle</td><td style="text-align:right;font-weight:600;color:#1B3A5C">${fmtRpFull(totalKomisi)}</td></tr>`;
    } else if (e.peran === 'Supir') {
        const trips = (DB.tripList || []).filter(t => {
            if (t.supirId !== e.id && t.supirNama !== e.nama) return false;
            return poMatchFilter({ tanggal: t.tanggal }, bulanSel, tahunSel);
        });
        totalKomisi = trips.reduce((s, t) => s + (t.upahSupir || 0), 0);
        detailRows = trips.map(t => `<tr><td>Supir</td><td>Trip ${t.tanggal || '-'} \u2014 ${t.tujuan || '-'}</td><td style="text-align:right">\u2014</td><td style="text-align:right;font-weight:600;color:#1B3A5C">${fmtRpFull(t.upahSupir || 0)}</td></tr>`).join('');
        if (!detailRows) detailRows = `<tr><td colspan="4" style="text-align:center;color:#888">Tidak ada trip di periode ini</td></tr>`;
    }

    const riwayatCairHTML = (e.riwayatCair || []).length
        ? `<div style="margin-top:18px;padding-top:10px;border-top:1px dashed #ccc">
            <div style="font-size:10px;text-transform:uppercase;color:#888;margin-bottom:6px;letter-spacing:.5px">Riwayat Pencairan Sebelumnya</div>
            ${(e.riwayatCair || []).map(rc =>
            `<div style="display:flex;justify-content:space-between;font-size:11px;padding:2px 0;border-bottom:1px dotted #eee">
                    <span style="color:#555">${rc.tanggal}</span>
                    <span style="font-weight:500">Rp ${rc.jumlah.toLocaleString('id-ID')}</span>
                </div>`).join('')}
           </div>` : '';
    ...
    <style>
      *{box-sizing:border-box;margin:0;padding:0}
      ...
      .co-name{font-size:16px;font-weight:bold;color:#0D2137;letter-spacing:1px}
      .co-sub{font-size:11px;color:#555;margin-top:3px}
      .kwt-box{border:2px solid #0D2137;padding:8px 16px;text-align:center;min-width:160px}
      .kwt-box .label{font-size:9px;text-transform:uppercase;letter-spacing:1px;color:#555}
      .kwt-box .no{font-size:14px;font-weight:bold;color:#0D2137;margin-top:2px}
      hr.tebal{border:none;border-top:2.5px solid #0D2137;margin:6px 0 2px}
      hr.tipis{border:none;border-top:1px solid #000;margin:2px 0 14px}
      .title{text-align:center;font-size:16px;font-weight:bold;letter-spacing:5px;margin:10px 0 16px;color:#0D2137}
      ...
      th{background:#0D2137;color:#FAC775;padding:8px 10px;text-align:left;font-size:11px}
      td{padding:8px 10px;border-bottom:1px solid #e8e8e8}
      .total-box{background:#f5f8ff;border:1.5px solid #0D2137;padding:14px 16px;display:flex;justify-content:space-between;align-items:center;margin-bottom:20px}
      .total-box .lbl{font-size:13px;color:#333}
      .total-box .amt{font-size:22px;font-weight:bold;color:#0D2137}
      .terbilang{background:#fffbf0;border:1px dashed #cca040;padding:8px 12px;font-size:11px;color:#555;margin-bottom:20px;font-style:italic}
      .foot{display:flex;justify-content:space-between;margin-top:36px;font-size:11px}
      .sign{text-align:center;width:200px}
      .sign-line{border-top:1px solid #000;padding-top:5px;margin-top:60px}
      .periode-badge{display:inline-block;background:#E8F0FA;border:1px solid #B8C8D8;padding:2px 10px;border-radius:3px;font-size:11px;color:#1B3A5C;font-weight:600}
    ...
    <div class="info-grid">
      ...
      <span class="lbl">Periode</span><span class="val"><span class="periode-badge">${periodeLabel}</span></span>
    ...
    <table>
      <thead><tr><th>Jabatan</th><th>Keterangan</th><th style="text-align:right">Rate</th><th style="text-align:right">Jumlah</th></tr></thead>
    ...
    <div class="terbilang">
      Terbilang: <strong>${terbilangRupiah(totalKomisi)} rupiah</strong>
    </div>
    ...
    <div style="margin-top:30px;padding-top:10px;border-top:1px dashed #ccc;font-size:10px;color:#aaa;text-align:center">
      Dokumen ini dicetak secara sistem oleh ${nPrs} pada ${tglCetak}
    </div>

    <div class="no-print" style="margin-top:24px;text-align:center">
      <button onclick="window.print()" style="padding:8px 24px;background:#0D2137;color:#FAC775;border:none;border-radius:4px;font-size:13px;cursor:pointer;font-family:inherit">&#x1F5A8; Cetak / Simpan PDF</button>
    </div>
    </body></html>`);
    win.document.close();
}
...
function cetakKwitansiKomisiKonsumen(id) {
    ...
    const detailRows = closedPO.map(p =>
        `<tr><td>Konsumen</td><td>${p.konsumen} (${p.tanggal || '-'})</td><td style="text-align:right">${fmtRpFull(rate)}/bundle</td><td style="text-align:right;font-weight:600;color:#1B3A5C">${fmtRpFull(p.bundle * rate)}</td></tr>`
    ...
    <style>
      *{box-sizing:border-box;margin:0;padding:0}
      ...
      td{padding:8px 10px;border-bottom:1px solid #e8e8e8}
      .total-box{background:#f5f8ff;border:1.5px solid #0D2137;padding:14px 16px;display:flex;justify-content:space-between;align-items:center;margin-bottom:20px}
      .total-box .lbl{font-size:13px;color:#333} .total-box .amt{font-size:22px;font-weight:bold;color:#0D2137}
      .terbilang{background:#fffbf0;border:1px dashed #cca040;padding:8px 12px;font-size:11px;color:#555;margin-bottom:20px;font-style:italic}
      .foot{display:flex;justify-content:space-between;margin-top:36px;font-size:11px}
