...
function cairkanKomisi(id) {
    const e = DB.entitas.find(x => x.id === id);
    if (!e) return;
    // outstanding: pakai max(0, komisiDibayar) agar komisiDibayar negatif (data corrupt) tidak inflate nilai
    ...
    // Hitung kasbon/hutang yang perlu dipotong
    const totalPengeluaran = (e.pengeluaranList || []).reduce((s, p) => s + (p.jml || 0), 0);
    const sudahDipotong = e.pengeluaranDipotong || 0;
    const sisaHutang = Math.max(0, totalPengeluaran - sudahDipotong);
    const cairBersih = Math.max(0, outstanding - sisaHutang);
    ...
    if (adaHutang) {
        ...
        e.riwayatCair.push({
            tanggal: today,
            jumlah: cairBersih,
            keterangan: `Komisi ${fmtRpFull(outstanding)} - kasbon ${fmtRpFull(sisaHutang)} = dibayar ${fmtRpFull(cairBersih)}`
        });
...
EATURES — Modal Actions, Buat PO, Settings, Export
